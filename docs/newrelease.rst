...
This is an advanced developer section that describes creation of a new release for cansrmapp and deploying
...
Be sure to update the version in ``cansrmapp/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' cansrmapp/__init__.py
...
From cansrmapp source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- cansrmapp-0.1.0-py2.py3-none-any.whl
    `-- cansrmapp-0.1.0.tar.gz
...
#. From https://github.com/idekerlab/cansrmapp click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/cansrmapp and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi cansrmapp
...
.. code-block::
    ...
    pip install cansrmapp


.. _Travis: https://travis-ci.com/idekerlab/cansrmapp
.. _main: https://github.com/idekerlab/cansrmapp/tree/main
